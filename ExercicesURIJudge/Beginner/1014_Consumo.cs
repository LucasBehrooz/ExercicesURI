...
namespace ExercicesURIJudge.Beginner
{
    class _1014_Consumo
    {
        ...
        {
            var X = int.Parse(Console.ReadLine(), CultureInfo.InvariantCulture);
            var Y = double.Parse(Console.ReadLine(), CultureInfo.InvariantCulture);
            var consumoMedia = X / Y;
            Console.WriteLine(consumoMedia.ToString("F3", CultureInfo.InvariantCulture) + " km/l");
        }
    }
}
